...
# This was mostly written by chatGPT, it matches generators to substations and creates the generation for the right
...
def load_substations():
    ...
    for sheet in NETWORK_SHEETS:
        sub_df = pd.read_excel("ETYS_B.xlsx", sheet_name=sheet, skiprows=1)
...
def create_gens(net, NGET_bus_lookup, substation_group):
    gen_df = pd.read_excel("ETYS_F.xlsx", sheet_name="TEC Register", skiprows=1)
